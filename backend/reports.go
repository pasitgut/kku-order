...
func workloadReport(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		query := db.Table("appointments AS a").
			Select("a.full_name AS full_name, MAX(a.position) AS position, COUNT(DISTINCT a.document_id) AS document_count, COUNT(*) AS appointment_count, SUM(CASE WHEN a.verified = 1 THEN 1 ELSE 0 END) AS verified_count").
		...
		if err := query.Group("a.full_name").Order("appointment_count DESC, a.full_name ASC").Limit(100).Scan(&rows).Error; err != nil {
